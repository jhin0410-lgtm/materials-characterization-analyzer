...
class PilotAuditConfig:
    ...
    def from_mapping(cls, payload: Mapping[str, Any]) -> "PilotAuditConfig":
        ...
        image = _remote_file(_mapping(pair, "image_file"), "landing_page_size_mb", "MB")
        label = _remote_file(_mapping(pair, "label_file"), "landing_page_size_mb", "MB")
        metadata = _remote_file(
            _mapping(pair, "processed_metadata_file"), "landing_page_size_kb", "KB"
        )
        config = cls(
            ...
            image_file=image,
            label_file=label,
            processed_metadata_file=metadata,
            hdf5=Hdf5Contract(
                ...
                allowed_label_values=tuple(int(value) for value in hdf5["allowed_label_values"]),
            ),
            training=TrainingReference(
                ...
                candidate_parent_patch_count=int(training["candidate_parent_patch_count"]),
            ...
            verified_training_input_name=_text(provenance, "verified_training_input_name"),
    ...
    def validate(self) -> None:
        if not re.fullmatch(r"[a-z0-9_]+", self.case_id):
            raise ValueError("case_id must contain lowercase letters, digits, and underscores.")
        ...
        if self.hdf5.patch_height % self.overlap.signature_block_size:
            raise ValueError("patch height must be divisible by signature_block_size.")
        if self.hdf5.patch_width % self.overlap.signature_block_size:
            raise ValueError("patch width must be divisible by signature_block_size.")
        ...
        for spec in (self.image_file, self.label_file, self.processed_metadata_file):
            if spec.file_id <= 0 or spec.landing_page_size <= 0:
                raise ValueError("Dryad file IDs and reported sizes must be positive.")
        if len({self.image_file.file_id, self.label_file.file_id, self.processed_metadata_file.file_id}) != 3:
            raise ValueError("Dryad file IDs must be distinct.")
        if self.training.shape[0] != (
            self.training.candidate_parent_patch_count * self.training.candidate_parent_count
        ):
            raise ValueError("training shape is inconsistent with candidate parent grouping.")
...
def normalize_dryad_file_metadata(
    ...
    """Normalize current Dryad API file metadata without guessing a missing digest."""
    if not isinstance(payload, Mapping):
    ...
    try:
        ...
        raise ValueError("Dryad file metadata does not contain a valid byte size.") from exc
    if size_bytes <= 0:
        raise ValueError("Dryad file byte size must be positive.")

    algorithm, digest = _extract_digest(payload)
    if algorithm != "md5":
        raise ValueError(f"Dryad file digest algorithm must be md5, not {algorithm!r}.")
    if not re.fullmatch(r"[0-9a-f]{32}", digest):
        raise ValueError("Dryad MD5 digest is invalid.")
...
def _extract_digest(payload: Mapping[str, Any]) -> tuple[str, str]:
    candidates = [payload.get("digest"), payload.get("checksum"), payload.get("md5")]
    for candidate in candidates:
        if isinstance(candidate, Mapping):
            algorithm = str(
                candidate.get("algorithm", candidate.get("type", candidate.get("scheme", "")))
            ).lower().strip()
            value = str(
                candidate.get("value", candidate.get("digest", candidate.get("checksum", "")))
            ).lower().strip()
        ...
        if isinstance(candidate, str) and candidate.strip():
            ...
            if ":" in text:
                ...
                return algorithm.strip(), value.strip()
...
def _remote_file(payload: Mapping[str, Any], size_key: str, unit: str) -> RemoteFileSpec:
