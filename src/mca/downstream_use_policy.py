"""Scientific-use policy for cross-repository characterization handoff bundles.

The policy records the strongest downstream use supported by the producer's
current evidence.  It does not prove that the use is scientifically valid;
consumers must independently enforce the same constraints and their own study
design requirements.
"""
...
DOWNSTREAM_USE_POLICY_SCHEMA_VERSION = "1.0"
...
class DownstreamUsePolicyError(ValueError):
    """Raised when a downstream-use policy is malformed or internally unsafe."""


def use_rank(value: str) -> int:
    """Return the ordered rank of one supported downstream-use level."""
    try:
        return DOWNSTREAM_USE_LEVELS.index(value)
...
def _required_text(policy: Mapping[str, object], field: str) -> str:
...
def _required_bool(policy: Mapping[str, object], field: str) -> bool:
    value = policy.get(field)
...
def validate_downstream_use_policy(
    ...
    *,
    ...
    schema_version = _required_text(policy, "schema_version")
    if schema_version != DOWNSTREAM_USE_POLICY_SCHEMA_VERSION:
        raise DownstreamUsePolicyError("unsupported downstream_use_policy schema_version")

    maximum_allowed_use = _required_text(policy, "maximum_allowed_use")
    maximum_rank = use_rank(maximum_allowed_use)
    feature_stage = _required_text(policy, "feature_stage")
    if feature_stage not in FEATURE_STAGES:
        raise DownstreamUsePolicyError(f"unsupported feature_stage: {feature_stage!r}")
    evidence_level = _required_text(policy, "evidence_level")
    ...
    review_status = _required_text(policy, "review_status")
    ...
    measurement_timing = _required_text(policy, "measurement_timing")
    if measurement_timing not in MEASUREMENT_TIMINGS:
        raise DownstreamUsePolicyError(
            f"unsupported measurement_timing: {measurement_timing!r}"
        )
    causal_design_validated = _required_bool(policy, "causal_design_validated")
    operational_validation_validated = _required_bool(
    ...
    if maximum_rank >= predictive_rank:
