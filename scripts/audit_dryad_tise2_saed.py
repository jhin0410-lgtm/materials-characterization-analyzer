...
def _fetch_json(url: str) -> dict[str, Any]:
    request = urllib.request.Request(url, headers={"User-Agent": "materials-characterization-analyzer/0.11"})
    try:
        with urllib.request.urlopen(request, timeout=90) as response:
            payload = json.load(response)
    except Exception as exc:  # network and malformed JSON remain fatal
    ...
    try:
        with urllib.request.urlopen(request, timeout=180) as response, target.open("wb") as handle:
            while True:
                ...
                total += len(chunk)
                if total > max_bytes:
                    raise DryadTiSe2AuditError("Dryad archive exceeds configured byte limit")
                md5.update(chunk)
                sha256.update(chunk)
                handle.write(chunk)
...
def _safe_member(info: zipfile.ZipInfo) -> str:
    ...
    if mode and (mode & 0o170000) == 0o120000:
        raise DryadTiSe2AuditError(f"symbolic link ZIP member rejected: {info.filename}")
    return path.as_posix()


def _classify(path: str, config: dict[str, Any]) -> tuple[str, str]:
    lowered = path.casefold()
    if any(lowered.startswith(prefix.casefold()) for prefix in config["required_experimental_prefixes"]):
        source_class = "experimental"
    elif any(lowered.startswith(prefix.casefold()) for prefix in config["required_simulation_prefixes"]):
        source_class = "simulation"
    elif any(lowered.startswith(prefix.casefold()) for prefix in config["supplementary_prefixes"]):
        source_class = "supplementary_or_mixed"
    else:
        source_class = "unresolved"
    suffix = PurePosixPath(path).suffix.casefold()
    if suffix in {value.casefold() for value in config["allowed_image_suffixes"]}:
        representation = "raster_image"
    elif suffix in {value.casefold() for value in config["allowed_table_suffixes"]}:
...
def run(config_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(title, str) or config["expected_title_fragment"].casefold() not in title.casefold():
        raise DryadTiSe2AuditError("Dryad dataset title does not match pinned source")
    doi = dataset.get("identifier") or dataset.get("doi")
    if isinstance(doi, str) and config["dataset_doi"].casefold() not in doi.casefold():
    ...
    with tempfile.TemporaryDirectory(prefix="dryad-tise2-") as temporary:
        ...
        byte_count, md5, sha256 = _download(_download_url(archive_item), archive_path, int(config["max_archive_bytes"]))
        ...
        with zipfile.ZipFile(archive_path) as archive:
            ...
            for info in infos:
                ...
                key = path.casefold()
                if key in seen:
                    raise DryadTiSe2AuditError(f"duplicate case-folded ZIP path: {path}")
                seen.add(key)
                if info.is_dir():
                    continue
                ...
                inventory.append({
                    ...
                    "diffraction_name_cue": any(cue in path.casefold() for cue in ("diff", "saed", "dp")),
    ...
    for prefix in config["required_experimental_prefixes"] + config["required_simulation_prefixes"]:
        if not any(path.casefold().startswith(prefix.casefold()) for path in paths):
    ...
    representation_counts = Counter(row["representation"] for row in inventory)
    calibration_cues = [path for path in paths if any(cue in path.casefold() for cue in ("calibration", "camera length", "pattern center", "pattern centre", "pixel size"))]
    summary = {
        "status": "archive_identity_and_experiment_simulation_partition_verified_but_calibration_and_acquisition_lineage_incomplete",
        "evidence_level": "Diagnostic",
        "record": {"doi": config["dataset_doi"], "title": title},
        "archive": {"name": config["expected_archive_name"], "bytes": byte_count, "md5": md5, "sha256": sha256, "integrity_test_passed": True},
        ...
        "experimental_image_count": sum(row["source_class"] == "experimental" and row["representation"] == "raster_image" for row in inventory),
        "simulation_image_count": sum(row["source_class"] == "simulation" and row["representation"] == "raster_image" for row in inventory),
        "diffraction_name_cue_count": sum(bool(row["diffraction_name_cue"]) for row in inventory),
        "hrtem_name_cue_count": sum(bool(row["hrtem_name_cue"]) for row in inventory),
        ...
        "readiness": {"external_validation_ready": False, "engineering_decision_ready": False},
        "processing": {
        ...
        "unresolved": [
            ...
            "whether TIFF/BMP files are direct lossless detector exports or publication-oriented derivatives",
    ...
    report_path.write_text(
        ...
        "The archive identity, integrity and top-level experimental/simulation separation are supported. "
        "Lossless detector provenance, acquisition identifiers, pattern centre, camera length and reciprocal calibration remain unresolved. "
        "No pixel data, inference, tuning, retraining or phase indexing was performed.\n",
        encoding="utf-8"
    )
    manifest_path.write_text(json.dumps(_manifest(output_dir, [summary_path, inventory_path, report_path]), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return summary


def main() -> int:
    parser = argparse.ArgumentParser(description="Audit the Dryad TiSe2 static-SAED/HRTEM archive without retaining source data.")
